/*Calculating 2nd order Taylor Series*/
/*Author: Mustaffa*/
/*Date: 5th July 2021*/
...
double ff(double x,double y) {
   return 2*y + 2*x*f(x,y) + f(x,y);
     }
//double fff(double x, double y) {//if question given already in dy/dx(need to close this instruction)
...
void taylor(double x,double y[]) {
    ...
    cout << "i\t\tx\t\ty " << endl;
    for ( i=0;i<=4;i++)    {
        y[i+1]=y[i]+h*f(x,y[i] ) + (h*h/2)*ff(x,y[i]) ;
...
cin >> x0 >> y[0];
cout<<"y(x+h) = y(x)+hy'(x) + h^2/2!y''(x)"<<endl;
cout<<"\n";
...
cout<<"\n";
cout<<"y("<<x0+h<<") = "<<y[0]<<"+"<<h*f(x0,y[0])<<"+"<<(h*h/2)*ff(x0,y[0])<<endl;
cout<<"y("<<x0+h<<") = "<<y[0]+h*f(x0,y[0])+(h*h/2)*ff(x0,y[0])<<endl;
cout<<"\n";
taylor(x0,y);
return 0;
